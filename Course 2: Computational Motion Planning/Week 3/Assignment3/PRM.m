...
for i = 2:nsamples
    ...
    k_index = order(1: min (k, i-1));
    for index=1: size(k_index)
        if (LocalPlanner(x, samples(:, k_index(index))) == true)
            nedges = nedges + 1;
            edges(nedges, :) = [k_index(index), i];
            edge_lengths(nedges) = distances(k_index(index));
        end
    end
